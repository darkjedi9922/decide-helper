...
class Session:
    ...
    def __init__(self, chatId):
        # В конструктор Net явно передаются пустые значения, потому что без такого
        # указания каким-то непонятным образом в конструктор передаются старые 
        # значения старого Net, который, вроде бы, уже удален.
        # 
        # Да даже если и не удален, все равно.
        # Вызывается новый конструктор нового объекта без параметров (они 
        # по-умолчанию равны []). Но при этом конструктор все равно в принципе
        # получает какие-то другие значения. Тем более старые. 
        # 
        # Это как вызвать функцию с одними параметрами, а в нее попадут совершенно
        # другие. Это очень странно. Возможно, питон каким-то образом закешировал
        # старые аргументы и передал их за меня.
        self._net = dhelper.Net(alternatives=[], factors=[])
        self._chatId = chatId
